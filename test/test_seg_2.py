...
model.load_state_dict(load_state_dict(os.path.join(mnt_dir, 'version_3/checkpoints/epoch=55-step=25199.ckpt'),
...
while True:
    ...
    if weather_real != w and tod_real != tod and not os.path.isfile(os.path.join(data_dir, "generations_seg_best", "{d}_{w}_seg_{f}".format(d=tod,f=filename, w=w))):
        ...
        source_file = "/mnt/Data/musa7216/10k/bdd100k/labels/sem_seg/colormaps/train/{f}.png".format(f=filename[:-4])
        destination_folder = "/mnt/Data/musa7216/seg_best_gtseg/"
        old_file_name = '{f}.png'.format(f=filename[:-4])
        new_file_name = '{w}_{t}_seg_{f}.png'.format(w=w, t=tod, f=filename[:-4])
        ...
        with torch.no_grad():
            ...
            if not os.path.isdir(os.path.join("/mnt/Data/musa7216/10k", "generations_seg_best")):
                os.makedirs(os.path.join("/mnt/Data/musa7216/10k", "generations_seg_best"))
            for i in range(num_samples):
                im = Image.fromarray(x_samples[i])
                im.save(os.path.join("/mnt/Data/musa7216/10k", "generations_seg_best", '{w}_{t}_seg_{f}'.format(f=filename, w=w, t=tod)))
