...
mnt_dir = "/mnt/Data/musa7216/cnet_results/new_lightning_logs"
...
model.load_state_dict(load_state_dict(os.path.join(mnt_dir, 'version_2/checkpoints/epoch=129-step=2989.ckpt'),
...
while True:
    ...
    if weather_real != w and tod_real != tod and not os.path.isfile(os.path.join(data_dir, "generations_edge_best_2", "{w}_{t}_edge_{f}".format(f=filename, w=w, t=tod))):
        ...
        destination_folder = "/mnt/Data/musa7216/10k/edge_best_gtseg_2/"
        ...
        with torch.no_grad():
            ...
            if det == 'None':
                ...
                detected_map = preprocessor(resize_image(input_image, detect_resolution), 100, 200)
            ...
            if not os.path.isdir(os.path.join(data_dir, "generations_edge_best_2")):
                os.makedirs(os.path.join(data_dir, "generations_edge_best_2"))
            for i in range(num_samples):
                im = Image.fromarray(x_samples[i])
                im.save(os.path.join(data_dir, "generations_edge_best_2", "{w}_{t}_edge_{f}".format(f=filename, w=w, t=tod)))
